...
MATH_MAP = {
    ...
    '𝗮': 'A', '𝗯': 'B', '𝗰': 'C', '𝗱': 'D', '𝗲': 'E', '𝗳': 'F', '𝗴': 'G', '𝗵': 'H', '𝗶': 'I', '𝗷': 'J', '𝗸': 'K', '𝗹': 'L', '𝗺': 'M', '𝗻': 'N', '𝗼': 'O', '𝐩': 'P', '𝗾': 'Q', '𝗿': 'R', 'ｓ': 'S', 'ｔ': 'T', 'ｕ': 'U', 'ｖ': 'V', 'ｗ': 'W', 'ｘ': 'X', 'ｙ': 'Y', 'ｚ': 'Z',
}
...
Send the Group ID you wish to toggle. 
        ...
        for line in lines:
            ...
            if len(p) == s.length:
                ...
                if len(word) > s.length:
                    w        self.candidates = ALL_WORDS.get(self.length, []).copy()
        self.count = 0
    def get_guess(self):
        if not self.candidates: return None
        if self.count == 0:
