...
MATH_MAP = {
    ...
    '𝗮': 'A', '𝗯': 'B', '𝗰': 'C', '𝗱': 'D', '𝗲': 'E', '𝗳': 'F', '𝗴': 'G', '𝗵': 'H', '𝗶': 'I', '𝗷': 'J', '𝗸': 'K', '𝗹': 'L', '𝐦': 'M', '𝐧': 'N', '𝗼': 'O', '𝐩': 'P', '𝗾': 'Q', '𝗿': 'R', 'ｓ': 'S', 'ｔ': 'T', 'ｕ': 'U', 'ｖ': 'V', 'ｗ': 'W', 'ｘ': 'X', 'ｙ': 'Y', 'ｚ': 'Z',
}
...
async def h_game(e, phone):
    ...
    if not data.get("enabled", True):
        return
    if e.chat_id not in game_sessions:
        return
    if is_blacklisted(e.chat_id):
        return
    s = game_sessions[e.chat_id]
    if not s.active:
        return
    ...
    async with s.lock:
        ...
        if "already guessed" in t or "someone has already guessed" in t:
            if time.time() - s.last_action < 1.5:
                return
        ...
        if not has_emojis and not is_game_over:
            return
            ...
            if len(p) == s.length:
                ...
                if len(word) == s.length:
                    ...
                    if key not in s.processed:
                        ...
                        s.processed.add(key)
                        new_info = True
                        # Check if this guess was from someone else
                        me = await e.client.get_me()
        ...
        if new_info:
            if time.time() - s.last_action < 1.5:
                return
            # Don't gamble: if opponent played, wait longer
            delay = 2 if data.get("mode") == "TURBO" else 4
            if opponent_played:
                delay += 3 # Add extra delay if someone else is playing
            ...
            if g:
                ...
                # Final check if game is still active and not over
                if s.active:
                    await e.client.send_message(e.chat_id, g.lower())
